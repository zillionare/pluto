...
async def score(bars: bars_dtype, code: str = None, frametype: str = 'day') -> Tuple:
    ...
    assert (frametype == 'day') or (frametype == 'min30'), "'frmetype' must choose string between 'day' and 'min30'!"
    if frametype == 'day':
        assert len(bars) >= 3, "must provide a day frametype array with at least 3 length!"
        limit_flag = (
        ...
        if limit_flag & ((bars["open"][1] - bars["close"][0]) / bars["close"][0] < 0.099):
        ...
        assert len(bars) >= 24, "must prrovide a min30 framtype array with at least 24 length!"
        ...
        second_day_end_index = np.where(bars['frame']==tf.combine_time(second_day, 15))[0].item()
        # 检测第二天开始日收盘价
        day_bars = bars[second_day_end_index:][::8]
        #获取当天涨停价
        first_limit_price = (
            await Stock.get_trade_price_limits(
                code, first_day, first_day
            )
        )[0][1]
        #获取第二天涨停价
        second_limit_price = (
            await Stock.get_trade_price_limits(
                code, second_day, second_day
            )
        )[0][1]
        ...
        if (price_equal(bars['close'][0], first_limit_price) and
            ...
            price = np.append(bars['open'][bars['frame']==second_open_time], day_bars['close'][1:])
